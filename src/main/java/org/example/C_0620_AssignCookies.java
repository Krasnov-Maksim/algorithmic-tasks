 ...
 */
public class C_0620_AssignCookies {
    public static void main(String[] args) {
        ...
        System.out.println(fastSolution(new int[]{1, 2, 3}, new int[]{1, 1}));
        System.out.println(fastSolution(new int[]{1, 2}, new int[]{1, 2, 3}));
    }
    ...
    private static boolean findMatching(int eagerSize, int[] cookieSize) {
        for (int i = 0; i < cookieSize.length; i++) {
            if (cookieSize[i] >= eagerSize) {
                cookieSize[i] = -1;
                return true;
            }
        }
    ...
    private static int fastSolution(int[] g, int[] s) {
        Arrays.sort(g);
        Arrays.sort(s);
        int counter = 0;
        int i = 0;
